...
(defn- maybe-delay
  "Returns a promise that resolves after `*load-delay-ms*` milliseconds,
   or nil when delay is 0. Callers should use `with-delay` to handle both cases."
  []
  (when-not (zero? *load-delay-ms*)
    (p/delay *load-delay-ms*)))

(defn- with-delay
  "Calls `(f)` after the simulated network delay. When `*load-delay-ms*` is 0,
   calls `f` synchronously (no promise wrapping). Otherwise chains via `p/then`."
  [f]
  (if-let [d (maybe-delay)]
    (p/then d (fn [_] (f)))
    (f)))
