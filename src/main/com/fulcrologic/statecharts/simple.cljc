...
(defn strict-env
  "Like `simple-env`, but enables W3C SCXML §4.4 strict block-error semantics:
   ...
   `::sc/errors-abort-siblings?` to `true` on their env."
  ([] (strict-env {}))
  ([extra-env]
   (assoc (simple-env extra-env) ::sc/errors-abort-siblings? true)))
...
(defn start!
  ...
  Returns true on success. Throws if there is a configuration problem."
  ([env chart-src]
