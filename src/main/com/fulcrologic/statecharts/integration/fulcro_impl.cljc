...
(defn resolve-alias-path [local-data alias]
...
(defn resolve-actors-in-path
  "Substitutes any :actor/xxx keywords in `path` with the ident of that actor in `local-data`'s :fulcro/actors."
  [local-data path]
  (let [actors (:fulcro/actors local-data)]
    (or
      (when (vector? path)
...
;(defmethod run-fulcro-data-op! :load [app penv {:keys [root component target abort-id
;                                                       params ok-event error-event]}]
;  (let [session-id (senv/session-id penv)
;        {:keys [actors] :as _session} (get-in (rapp/current-state app) (statechart-session-ident session-id))
;        Query      (cond
;                     (contains? actors component) (some->> component
;                                                    (get actors)
;                                                    (rc/registry-key->class))
;                     (rc/component-class? component) component
;                     (rc/registry-key->class component) (rc/registry-key->class component))]
;    (df/load! app root Query (cond->
;                               {:params       params
;                                :ok-action    (fn [env]
;                                                (when ok-event
;                                                  (sp/send! (::sc/event-queue penv) penv {:event             ok-event
;                                                                                          :data              (dissoc env :state :app)
;                                                                                          :source-session-id session-id})))
;                                :error-action (fn [env]
;                                                (when error-event
;                                                  (sp/send! (::sc/event-queue penv) penv {:event             error-event
;                                                                                          :data              (dissoc env :state :app)
;                                                                                          :source-session-id session-id})))}
...
(deftype FulcroWorkingMemoryStore [fulcro-app]
