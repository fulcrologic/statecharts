...
makes a decision about where to transition to. In the SCXML standard they are coded as a state that includes nothing
...
This is a bit noisy, isn't immediately obvious to the reader, and the predicates are code, so there is no way
for them to easily be emitted to a diagram. So there is a macro version of `choice`
in the `convenience-macros` namespaces:
