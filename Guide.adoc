...
There is a predefined `WorkingMemoryDataModel` in `data-model.working-memory-data-model` that stores scoped data for
states within the session working memory, allowing the data, for example, to be persisted with the state of
a running session when pausing it for later revival.

There is also a `FlatWorkingMemoryDataModel` (same ns) that puts all data into a single scope (it can still be a
