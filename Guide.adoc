...
When executable content (`&lt;script&gt;`, `&lt;assign&gt;`, `&lt;send&gt;`, `&lt;foreach&gt;`, ...) raises an error,
the W3C SCXML specification mandates two things:
...
(def env
  (-> (build-my-env)            ;; your registry, data model, etc.
      (assoc :com.fulcrologic.statecharts/strict-block-errors? true)))
------
...
In non-strict mode the same chart still queues the `error.execution` event and routes to `:recovery`,
but the `assign` would also have run before the transition fires.
...
The `com.fulcrologic.statecharts.visualization` namespace provides tools for visualizing and
interactively simulating statecharts. The visualizer renders your chart as a UML-style state
diagram and, when connected to a running application, lets you observe live session state in
real time.

There are two main components:

- **Simulator** — A pure CLJ/CLJS API for stepping through a statechart interactively at the
  REPL or in a browser UI, using the real processor but with a mock execution model.
- **Visualization Server** — A standalone Ring server (or embeddable Pathom resolvers) that
  exposes chart definitions and live session state to the browser visualizer.

== The Simulator API

The simulator (`com.fulcrologic.statecharts.visualization.simulator`) runs your chart using the
real processor but with a custom execution model: guard predicates become user-toggleable
booleans, and all other executable content (scripts, assigns, sends) is a noop. This lets you
step through the chart's control flow without any real side-effects.
...
By default the visualizer uses element IDs and transition event names as labels. You can
override these with two metadata keys on your chart elements:
...
The easiest way to visualize a chart is to run the standalone server. It serves the browser UI
and exposes the Pathom API in one step.

The server supports three modes:

1. **Static mode** — renders a chart definition passed on the command line; no live session data.
2. **Connected/REPL mode** — connects to a running nREPL server and pulls chart definitions and
   session state from your live application.
3. **Proxy mode** — forwards Pathom queries to a remote server that already has the resolvers
   installed.
...
If you already have a Ring + Pathom server you can add the visualization resolvers directly,
giving the browser UI access to your live chart registry and working memory store.
...
When the server has a working memory store available, the browser UI can display which states
are currently active in any running session. The `search-for-sessions` multimethod lets you
plug in your own session-discovery logic.
...
The `search-string` argument comes from the browser's session search box. You can filter by
user name, session ID prefix, or any application-specific criterion. The default implementation
logs a warning and returns an empty vector.

== POST API for External Tools

The resolvers expose a `send-event` Pathom mutation that lets external tools trigger state
transitions without a UI. This is useful for scripted testing or integration from non-Clojure
environments.
...
and `::sc/processor` available. Static-mode standalone servers do not support this mutation.
