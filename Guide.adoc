...
| `sroute/busy? [env data]` | Deep recursive busy check (guard condition).
...
`sfro/busy?`:: A `(fn [env data] boolean?)` predicate. When true, routing away from this target is denied. Forms with `form-state` are auto-guarded (dirty form = busy).
