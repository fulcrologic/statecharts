...
as you simply want to run state charts all in the same JVM in RAM. That namespace also includes helper
...
The library includes an async-capable execution engine built on https://github.com/funcool/promesa[promesa] that allows
expressions to return promises the algorithm awaits. When all expressions are synchronous, the overhead is minimal.
When an expression returns a promise, the algorithm parks until it resolves before continuing.

Both `start!` and `process-event!` may return promises instead of plain values. Callers must handle both cases.
The easiest approach is to use `promesa.core/let` or `promesa.core/then` which handle both synchronous values and
promises transparently.
...
The operations `fop/load` and `fop/invoke-remote` described above use `send!` to place completion events on the
external event queue. This means the statechart rests in an intermediate state between starting the operation and
receiving the completion event. With async operations, the chart transitions atomically within a single
`process-event!` call.

The `com.fulcrologic.statecharts.integration.fulcro.async-operations` namespace (aliased as `afop`) provides
async-aware versions of these operations that return promesa promises and use `raise` (internal queue) instead of
`send!` (external queue).
...
`(afop/await-load env query-root component-or-actor options)` issues a remote load and returns a promise that
resolves when the load completes.

Parameters:

* `env` - The statechart environment (first arg to expressions)
* `query-root` - A keyword or ident for the root of the remote query
* `component-or-actor` - A Fulcro component, or a keyword naming a known actor
* `options` - Same as `fop/load` options:
** `::sc/ok-event` - Event to raise when the load succeeds
** `::sc/error-event` - Event to raise when the load fails
** `::sc/ok-data` - Extra data to include in the ok event
** `::sc/error-data` - Extra data to include in the error event
...
`(afop/await-mutation env txn options)` invokes a remote mutation and returns a promise that resolves when the
mutation completes.
...
* `options` - Same as `fop/invoke-remote` options:
** `:ok-event` - Event to raise on success
** `:error-event` - Event to raise on failure
** `:ok-data` - Extra data for the ok event
** `:error-data` - Extra data for the error event
** `:target` - Data-targeting path (vector, alias keyword, or actor-prefixed)
...
(state {:id :saving}
  (on-entry {}
    (script {:expr (fn [env data]
                     (let [form-data (resolve-actor data :actor/form)]
                       (afop/await-mutation env [(save-user form-data)]
...
==== Comparison: Send-based vs Raise-based

With standard operations, intermediate states are visible:

[source]
-----
;; Using fop/load - chart rests in :loading
(state {:id :loading}
  (on-entry {}
    (script {:expr (fn [env data]
                     [(fop/load :user User {::sc/ok-event :event/loaded})])}))
  ;; Chart is in :loading state until external event arrives
  (transition {:event :event/loaded :target :viewing}))
-----

With async operations, the transition is atomic:

[source]
-----
;; Using afop/await-load - atomic transition
(state {:id :loading}
  (on-entry {}
    (script {:expr (fn [env data]
                     (afop/await-load env :user User {::sc/ok-event :event/loaded}))}))
  ;; Event is raised internally, processed in same microstep
  (transition {:event :event/loaded :target :viewing}))
-----

The async version never observably rests in `:loading` — it transitions directly to `:viewing` when the promise
resolves.

NOTE: The existing `fop/load` and `fop/invoke-remote` operations remain unchanged and continue to work with both
sync and async execution engines. Use `afop/await-load` and `afop/await-mutation` only when you need atomic transitions.
